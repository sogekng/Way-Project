...
window.addEventListener('wheel', function(event) {
  var elemento = document.querySelectorAll('.backg');

  event.preventDefault();
  escala += event.deltaY * -0.01;
  escala = Math.min(Math.max(.125, escala), 4);

  if(escala >= 4){
    console.log('Acima de ' + escala)
  }else{
    console.log('Abaixo de ' + escala)
  }
});
                  ...
                  <div className='textMenu'><strong>CONTATOS</strong></div>
                    <input type="checkbox" className="checkbox" id="chk" onClick={toggleTheme}/>
                      <label className="label" for="chk">
                        ...
                        <span class="material-symbols-outlined" style={{color: '#1b2431'}}>sunny</span>
                        <span class="material-symbols-outlined" style={{color: '#e4dcc0'}}>bedtime</span>
                      </label>
              </header>
            </div>
        </ThemeProvider>
